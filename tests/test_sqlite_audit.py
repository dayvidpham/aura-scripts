...
def _make_event(
    epoch_id: str = "ep-1",
    event_type: EventType = EventType.PhaseAdvance,
...
class TestEnsureSchema:
    ...
    async def test_table_has_required_columns(self, tmp_path: Path) -> None:
        db_path = tmp_path / "audit.db"
        await ensure_schema(db_path)
    ...
    async def test_idempotent_double_call(self, tmp_path: Path) -> None:
        ...
        db_path = tmp_path / "audit.db"
        await ensure_schema(db_path)
        await ensure_schema(db_path)  # Second call must be a no-op
...
class TestRecordEvent:
    ...
    async def test_stores_all_fields(self, tmp_path: Path) -> None:
        ...
        event = _make_event(
            epoch_id="ep-all",
            event_type=EventType.ConstraintCheck,
        ...
        assert row[0] == "ep-all"
        assert row[1] == EventType.ConstraintCheck.value
...
class TestQueryEvents:
    ...
    async def test_results_in_chronological_order(self) -> None:
        ...
        assert events[0].phase == PhaseId.P9_Slice
        assert events[1].phase == PhaseId.P10_CodeReview
